...
//gcc -o wavecuepoint wavecuepoint.c
//tb/1903
//add main()
//arg1: wav file
//arg2: text input
//arg3: wav output
//format / whitespace / no-op
//define int outside of for(int i=...)
...
int addMarkersToWaveFile(char *inFilePath, char *markerFilePath, char *outFilePath, enum CuePointMergingOption mergeOption)
{
	...
	while (1)
	{
		...
		else if (strncmp(&nextChunkID[0], "cue ", 4) == 0)
		{
			...
			uint32_t cuePointIndex = 0;
			for (cuePointIndex = 0; cuePointIndex < cuePointsCount; cuePointIndex++)
			{
...
//	uint16_t bytesPerSample = bitsPerSample / 8;
	...
	uint32_t i=0; //used as counter variable from here on

	for (i = 0; i < cueLocationsCount; i++)
	{
	...
	for (i = 0; i < littleEndianBytesToUInt32(cueChunk.cuePointsCount); i++)
	{
...
int main(int argc, char *argv[])
{
	if(argc!=4)
	{
		fprintf(stderr, "need 3 arguments: input.wav cuepoints.txt output.wav\n");
		fprintf(stderr, "wave files must be of type RIFF.\n");
		fprintf(stderr, "text file must contain one integer (sample position) per line.\n");
		return 1;
	}

	enum CuePointMergingOption replace;
	replace=ReplaceAnyExistingCuePoints;
	addMarkersToWaveFile(argv[1], argv[2], argv[3], replace);
	return 0;
}
/*
test: add 3 cue points from a.txt to a.wav, write to b.wav
...
Sections    : 1
Seekable    : TRUE
Duration    : 00:00:12.408
Signal Max  : 1818 (-25.12 dB)
...
$ ./wavecuepoint a.wav a.txt b.wav
Reading input wave file.
Got Format Chunk
Got Data Chunk
Reading markers file.
Read 3 cue locations from markers file.
Preparing new cue chunk.
Writing output file.
Finished.
...
cue  : 76
  Count : 3
   Cue ID :  1  Pos :     0  Chunk : data  Chk Start : 0  Blk Start : 0  Offset : 44100
   Cue ID :  2  Pos :     0  Chunk : data  Chk Start : 0  Blk Start : 0  Offset : 123456
   Cue ID :  3  Pos :     0  Chunk : data  Chk Start : 0  Blk Start : 0  Offset : 547199
data : 1094400
End
